...
- The ship's booster fire is no longer always on: the flame tori and nozzle glow discs from `ship.glb` (Material.003/Material.002 — the `Torus*`/`Circle*` nodes) are hidden while idle and only appear while thrusting (W, incl. boost) or during autopilot warp. The GLB's baked clip is now partially played for the first time — but only the pure flicker tracks (`*.scale` + `*.morphTargetInfluences`, flame breathing/flare); the roll motion is baked into **every** position/quaternion track of the clip including the fire nodes themselves (180° flips around the ship's long axis), so all of those are excluded (`components/Ship.tsx`).
