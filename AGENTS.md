...
Built with **Next.js 16 (App Router) + React 19 + TypeScript + React Three Fiber (`@react-three/fiber` + `@react-three/drei`) + `@react-three/postprocessing`**, Tailwind CSS 4, and Zustand for UI state. Planet bodies come from a single Sketchfab GLB (`planet_of_phoenix.glb`, a phoenix-textured planet), cloned per project and tinted by its project palette.
...
| `components/Planet.tsx` | One planet: clones the phoenix mesh from `planet_of_phoenix.glb`, **clones its materials per planet** (so shared GLB nodes are never mutated) and tints them from `palette` (body hue → `palette.mid` lerped toward white, `emissive` → `palette.emissive`/`atmosphere`, roughness/metalness/`toneMapped` from a per-`type` preset). Adds a back-side additive fresnel atmosphere glow (`createAtmosphereMaterial`, color `palette.atmosphere`, distance-faded) and optional `rings`. Lava/tech planets pulse their emissive (`toneMapped=false` so Bloom catches it). Html label |
...
| `lib/planets.ts` | **Single source of truth** for projects: name, description, link, tags, palette, orbit/radius/speed/phase/seed, optional rings |
| `lib/geometry.ts` | Atmosphere shader + ring texture + starfield material |
...
- **Projects / content / planet look:** `lib/planets.ts` (add/edit entries in the `THEMES` array — id, name, description, link, tags, palette, rings). Every planet is a clone of the single phoenix mesh from `public/planets/planet_of_phoenix.glb` (`Phoenix_LOD0__0`, body unit radius 22 — `BODY_UNIT_RADIUS` in `Planet.tsx` scales it to `radius`). Each planet's look is driven by its `palette` + `type`: body tinted toward `palette.mid`, emissive = `palette.emissive`/`atmosphere`, plus a `TYPE_PRESETS` table in `components/Planet.tsx` (roughness/metalness/emissive/`toneMapped`/pulse). A back-side additive fresnel atmosphere glow (`createAtmosphereMaterial` in `lib/geometry.ts`, color `palette.atmosphere`) sits ~12% larger than the body and fades with distance. Set `rings: true` on a theme to add a ring (`createRingTexture` in `lib/geometry.ts`, tinted `palette.high`). **System scale:** orbits are `6000 + i * 2800` (≈6000–36800), planet radii `(34 + (i % 4) * 14) * 3.5` (≈119–266 units), orbital speed `1 / Math.sqrt(orbit)` — bump these in the `PLANETS` map to resize the whole system. The sun (`Star.tsx`), skybox (`NightSky.tsx`), starfield (`Starfield.tsx`) and camera far plane (`Experience.tsx`) are sized to match.
